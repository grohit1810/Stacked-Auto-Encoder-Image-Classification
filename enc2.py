...
class NeuralNetwork(object):
    ...
    def train(self, inputs, targets, validation_data, num_epochs, regularizer_type=None):
        ...
        for k in xrange(num_epochs):
            for i in xrange(len(inputs)):
                # Forward pass
                h_a, ys = self._feed_forward(inputs[i])
                
                
                #loss += -np.log(probs[targets[i], 0])

                # Backpropagation
                dWxh, dWhy, dbh, dby = self._back_propagation(inputs[i], targets[i], h_a, ys)
            ...
            for i in xrange(len(validation_inputs)):
                # Forward pass
                #print i
                h_a, ys = self._feed_forward(validation_inputs[i])
                #loss += -np.log(probs[targets[i], 0])

                # Backpropagation
                dWxh, dWhy, dbh, dby = self._back_propagation(validation_inputs[i], validation_targets[i], h_a, ys)
    ...
    def predict(self, X):
        ...
        # return probs
        #print self.Wxh
        pickle.dump(self.Wxh,open("Wxh1.pkl","wb"))
        pickle.dump(self.bh,open("bh1.pkl","wb"))
        #pickle.dump(self.l,open("l.pkl","wb"))
...
if __name__ == "__main__":
    ...
    nn = NeuralNetwork(16,8,16)
    
    nn.train(l[:1600], l[:1600], ([], []), 20, regularizer_type='L2')
    '''print "Input: ",l[900]
    nn.predict(l[500])'''
